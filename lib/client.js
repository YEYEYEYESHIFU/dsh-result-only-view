...
// (native collapsed rows) and clicking again collapses them. Hovering never
// expands anything — reveal is strictly click-driven.
//
  ...
  factory: (require) => {
    ...
    const BASE_HIDE_CSS = `
      ...
      /* Context injections / recollections (context, context-recall nodes):
         ...
         turn. Revealed again through the turn trace (click only). */
    ...
    /** Keys currently forced visible: click reveals only. */
    ...
    function apply(ctx) {
      ...
      function TurnTrace(props) {
        ...
        // Expand/collapse strictly on click; no hover reveal. Sweeping the
        // pointer across the trace line must never reshuffle the layout.
        return React.createElement(
          "button",
          {
            type: "button",
